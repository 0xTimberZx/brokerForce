...
function defaultTierFor(classA: string, classB: string): PairTier {
  ...
  // from full treatment regardless of any volume/TVL they might have --
  // this is the one tier assignment this script CAN make deterministically,
  // since it depends only on asset class, not on pool data.
  if (classA === "stable" && classB === "stable") return "excluded-stable";
