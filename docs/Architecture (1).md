...
**Refresh cadence scales with window length:** shorter windows are refreshed more often, since they're more sensitive to recent data. Proposed mapping (confirm before locking): **30d → every 30 minutes, 90d → hourly, 200d → every 4 hours.** A 200d average barely moves in a few hours; a 30d figure can shift meaningfully in 30 minutes, so cadence should match sensitivity rather than refreshing everything on one global timer.
...
**Pair Engine tiering: full depth only for active pairs.** Not every generated pair combination gets the same analytical effort. Pairs with real trading/pool activity ("popular" pairs) get the full statistical profile. Other generated combinations remain explorable, but surface a visibly limited/lighter analysis rather than the full metric set — this keeps compute focused and keeps Pair Explorer from filling with noise from combinations no one would actually LP.
